...
function Avatar({ member, size = 64 }) {
  ...
  return (
    <div style={{
      ...
      fontSize: size * 0.3, fontWeight: '700', color: 'white', flexShrink: 0,
...
function getBio(member, section) {
  return (member.bios && member.bios[section]) || member.bio
}

// Large card for Executive Board
function ExecutiveCard({ member }) {
  ...
  return (
    <div
      ...
      style={{
        background: hovered ? 'rgba(37,99,235,0.07)' : 'rgba(255,255,255,0.025)',
        border: hovered ? '1px solid rgba(37,99,235,0.35)' : '1px solid rgba(255,255,255,0.08)',
        borderRadius: '20px',
        padding: '32px',
        transition: 'all 0.3s ease',
        transform: hovered ? 'translateY(-5px)' : 'translateY(0)',
        boxShadow: hovered ? '0 24px 64px rgba(37,99,235,0.15)' : '0 4px 24px rgba(0,0,0,0.2)',
        display: 'flex',
    ...
    >
      ...
      <div>
        <h3 style={{ fontSize: '18px', fontWeight: '700', color: 'white', letterSpacing: '-0.01em', marginBottom: '6px' }}>
          {member.name}
        </h3>
        <span style={{
          ...
          fontSize: '11px', fontWeight: '600', color: '#60a5fa', letterSpacing: '0.03em',
        }}>
          {member.role}
        </span>
      </div>
      ...
      <div style={{
        ...
        background: 'rgba(255,255,255,0.04)', borderRadius: '20px', padding: '4px 10px',
      }}>
      ...
      <p style={{ color: '#9ca3af', fontSize: '13px', lineHeight: 1.7, maxWidth: '260px' }}>
        {getBio(member, 'executive')}
      </p>
...
// Standard card for department members
function MemberCard({ member, section }) {
  const [hovered, setHovered] = useState(false)
  return (
    <div
      onMouseEnter={() => setHovered(true)}
    ...
    >
      <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h3 style={{ fontSize: '15px', fontWeight: '700', color: 'white', letterSpacing: '-0.01em', marginBottom: '3px' }}>
            {member.name}
          </h3>
          <p style={{ fontSize: '11px', fontWeight: '600', color: '#3b82f6', letterSpacing: '0.02em', marginBottom: '3px' }}>
            {member.role}
          </p>
      ...
      {member.bio && (
        <p style={{ color: '#9ca3af', fontSize: '12px', lineHeight: 1.7, margin: 0 }}>
          {getBio(member, section)}
        </p>
      )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
          {member.projects.map(p => (
            <span key={p} style={{
              ...
              borderRadius: '5px', padding: '2px 7px', fontSize: '10px', color: '#9ca3af', fontWeight: '500',
            }}>{p}</span>
          ))}
        </div>
      )}
...
export default function Team() {
  const { lang } = useLang()
  const tr = t[lang]
  const executiveBoard = members.filter(m => m.isExecutive)
  const founders = members.filter(m => m.isFounder)

  return (
    <div style={{
      background: 'linear-gradient(180deg, #05050f 0%, #0a0a1a 40%, #0d0d2b 100%)',
      minHeight: '100vh',
      paddingTop: '88px',
      position: 'relative',
      overflow: 'hidden',
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '8%', right: '-8%', width: '600px', height: '600px', background: 'radial-gradient(circle, rgba(37,99,235,0.07) 0%, transparent 60%)', borderRadius: '50%' }} />
        <div style={{ position: 'absolute', bottom: '20%', left: '-8%', width: '500px', height: '500px', background: 'radial-gradient(circle, rgba(139,92,246,0.06) 0%, transparent 60%)', borderRadius: '50%' }} />
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '800px', height: '800px', background: 'radial-gradient(circle, rgba(6,182,212,0.03) 0%, transparent 60%)', borderRadius: '50%' }} />
      </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px 100px', position: 'relative', zIndex: 1 }}>
        ...
        <section style={{ marginBottom: '80px' }}>
          ...
          <div style={{
            ...
            {executiveBoard.map(m => <ExecutiveCard key={m.id} member={m} />)}
          </div>
        </section>

        {/* Department Sections */}
        {DEPARTMENT_KEYS.map(dept => {
          const deptMembers = dept.key === 'Founders'
            ? founders
            : members.filter(m => m.department === dept.key || (dept.key === 'Engineering' && m.showInEng))
          ...
          return (
            <section key={dept.key} style={{ marginBottom: '64px' }}>
              ...
              <div style={{
                display: 'grid',
                gridTemplateColumns: dept.key === 'Founders'
                  ? 'repeat(auto-fill, minmax(300px, 1fr))'
                  : 'repeat(auto-fill, minmax(280px, 1fr))',
                gap: '16px',
              }}>
                {deptMembers.map(m => <MemberCard key={`${dept.key}-${m.id}`} member={m} section={dept.bioKey} />)}
              </div>
            </section>
          )
        })}
